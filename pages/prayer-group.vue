...
const presentPercent = computed(() =>
  percentage(summary.value.present, summary.value.total),
);
...
async function fetchTrail() {
  ...
  try {
    ...
    errorMessage.value =
      error?.data?.message ||
      error?.message ||
      "Unable to load prayer meeting participation.";
  } finally {
    loading.value = false;
  }
}
  ...
  <section class="space-y-5">
    ...
    >
      <div>
        ...
        >
          Prayer meeting participation
        </h1>
        <p class="mt-2 max-w-2xl text-sm leading-6 text-gray-500">
          Prayer meeting participation history
        </p>
      </div>
    </div>
    ...
    <UCard class="border border-gray-200 bg-white shadow-sm">
      <div
        class="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)] lg:items-center"
      >
        <div class="mx-auto flex flex-col items-center">
          <div
            class="relative grid h-44 w-44 place-items-center rounded-full shadow-inner"
            :style="chartStyle"
          >
            <div
              class="grid h-28 w-28 place-items-center rounded-full bg-white text-center shadow-sm"
            >
              <div>
                <p class="m-0 text-3xl font-semibold text-gray-950">
                  {{ presentPercent }}%
                </p>
                <p
                  class="m-0 text-xs font-medium uppercase tracking-wide text-gray-500"
                >
                  Present
                </p>
              </div>
          ...
          <div class="mt-4 flex gap-3 text-xs font-medium">
            <span class="inline-flex items-center gap-1 text-green-800"
              ><span
                class="h-2.5 w-2.5 rounded-full bg-green-800"
              />Present</span
            >
            <span class="inline-flex items-center gap-1 text-[#a83632]"
              ><span
                class="h-2.5 w-2.5 rounded-full bg-[#a83632]"
              />Absent</span
            >
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-2 sm:gap-3">
          <div class="rounded-2xl border border-gray-200 bg-gray-50 p-3 sm:p-4">
            ...
            <p
              v-else
              class="mb-0 mt-2 text-xl font-semibold text-gray-950 sm:text-2xl"
            >
              {{ summary.total }}
            </p>
          </div>
          <div class="rounded-2xl border border-gray-200 bg-gray-50 p-3 sm:p-4">
            ...
            <p
              v-else
              class="mb-0 mt-2 text-xl font-semibold text-green-800 sm:text-2xl"
            >
              {{ summary.present }}
            </p>
          </div>
          <div class="rounded-2xl border border-gray-200 bg-gray-50 p-3 sm:p-4">
            ...
            <p
              v-else
              class="mb-0 mt-2 text-xl font-semibold text-[#a83632] sm:text-2xl"
            >
              {{ summary.absent }}
            </p>
          </div>
    ...
    <UCard class="border border-gray-200 bg-white shadow-sm">
      <div class="mb-4">
        ...
        <p class="m-0 mt-1 text-xs text-gray-500">
          Select a month-year range for prayer meeting records.
        </p>
      </div>
      <div
        class="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto_auto] md:items-end"
      >
    ...
    >
      <div class="py-10 text-center">
        ...
        <p class="mx-auto max-w-sm text-sm leading-6 text-gray-500">
          Your prayer meeting participation will appear here after attendance is
          recorded.
        </p>
      </div>
    </UCard>
        ...
        >
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2">
              <span
                class="rounded-full px-2.5 py-1 text-xs font-semibold capitalize ring-1"
                :class="statusClasses(item.participationStatus)"
              >
                {{ displayValue(item.participationStatus) }}
              </span>
              <span
                class="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-semibold capitalize text-gray-700"
              >
            ...
            <h2
              class="mb-0 mt-3 truncate text-base font-semibold text-gray-950"
            >
          ...
          >
            <div>
              <p class="m-0 text-xs text-gray-500">Prayer meeting</p>
