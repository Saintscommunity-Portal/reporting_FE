...
async function fetchFollowup() {
  const response = await request(`/followups/${route.params.id}`, { method: 'GET' })
  followup.value = response?.data || null
}

async function fetchReports() {
  const response = await request(`/followups/${route.params.id}/reports`, { method: 'GET', query: { per_page: 100 } })
...
async function refresh() {
  ...
  try {
    ...
    errorMessage.value = error?.data?.message || error?.message || 'Unable to load follow up reports.'
  } finally {
    loading.value = false
  }
}
...
function askDeleteReport(report) {
  deleteTarget.value = { id: report.id, label: report.targetName || 'this report' }
...
async function confirmDelete() {
  ...
  try {
    ...
    errorMessage.value = error?.data?.message || error?.message || 'Unable to delete report.'
  } finally {
    saving.value = false
  }
}
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p class="m-0 mt-2 text-sm text-gray-500">
              {{ displayDate(followup?.timeFrom) }} to {{ displayDate(followup?.timeTo) }}
            </p>
          </div>
        ...
        <div class="modal-dark space-y-4 bg-gray-950 p-5 text-white">
          <div>
            ...
            <p class="m-0 mt-2 text-sm text-gray-300">
              This will permanently remove {{ deleteTarget?.label || 'this report' }} from your follow up records.
            </p>
          </div>
          <div class="flex justify-end gap-2">
